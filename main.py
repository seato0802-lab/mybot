...
TOOL_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRH53TZ7iL7EFXNhkGTmRBS0JdE6oAjex51ape3cqOoXnuoR7RGATJlq_TaLupYmT4YJB2Luaa5NwXx/pub?gid=0&single=true&output=csv"
WEAPON_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRH53TZ7iL7EFXNhkGTmRBS0JdE6oAjex51ape3cqOoXnuoR7RGATJlq_TaLupYmT4YJB2Luaa5NwXx/pub?gid=793378898&single=true&output=csv"
...
async def list_cmd(interaction: discord.Interaction):
    ...
    for name, data in tasks_data.items():
        msg += f"・**{name}**：{data['time'].strftime('%H:%M')}\n"
...
async def autocomplete_name(interaction: discord.Interaction, current: str):
    return [app_commands.Choice(name=n, value=n) for n in tasks_data.keys() if current.lower() in n.lower()]

# =========================
# /craft（個人表示）＋autocomplete
# =========================
...
async def craft_cmd(interaction: discord.Interaction, category: app_commands.Choice[str], type: str, item: str, count: int):
    ...
    def find_col(cols, target):
        for c in cols:
            ...
            if target in c.replace("\u3000", "").strip(): return c
        return None
...
async def autocomplete_type(interaction: discord.Interaction, current: str):
    ...
    types = ["小型", "大型", "その他"] if category == "道具" else ["弾", "武器", "アタッチメント", "その他"]
...
async def autocomplete_item(interaction: discord.Interaction, current: str):
    ...
    urls = [TOOL_URL] if category == "道具" else [WEAPON_URL] if category == "武器" else [TOOL_URL, WEAPON_URL]
    candidates = []
    ...
    for url in urls:
        ...
        columns = sheet[0].keys()
        name_col = next((c for c in columns if "名前" in c.replace("\u3000", "").strip()), None)
        type_col = next((c for c in columns if "種別" in c.replace("\u3000", "").strip()), None)
...
async def check_tasks():
    ...
    for name, data in tasks_data.items():
        ...
        if notify_time <= now:
            ...
            if channel: await channel.send(f"@here **{name}** の受注15分前です！")
...
async def start():
    ...
    if not TOKEN: print("DISCORD_TOKEN not set!"); return
    while True:
        try: await bot.start(TOKEN)
        except Exception as e: print("Error:", e); await asyncio.sleep(5)
