...
def get_user_lock(uid: int) -> asyncio.Lock:
    ...
    # 使われていない古いロックを間引く（locked のものは残す）
    if len(_user_locks) > _USER_LOCKS_MAX:
        # 余剰分だけ掃除。locked はスキップ。
...
class SheetsStore:
    ...
    run_in_executor で呼ばれても同時実行にならないように Lock で直列化する。

    修正:
      - user_id -> row_index のマップを保持して upsert を O(1) 寄りにする
    """
    def __init__(self):
        self._lock = Lock()
        self.gc = None
        ...
        self._uid_to_row: dict[int, int] = {}  # user_id -> row index(1-based)
    ...
    def _load_users_and_index(self):
        with self._lock:
            ...
            # すべての行を取得して row index を作る
            all_values = self.ws_users.get_all_values()  # 1行目=ヘッダ
            users = {}
            uid_to_row = {}
    ...
    def upsert_user(self, u: dict):
        with self._lock:
            ...
            if idx is None:
                # append_row したら最後に追加されるはずなので、追加後に used 行数で推定
                self.ws_users.append_row(values)
                used_rows = len(self.ws_users.get_all_values())
                self._uid_to_row[uid] = used_rows
            else:
                # ✅ Z超え対応のA1記法で範囲を作る
                start_a1 = rowcol_to_a1(idx, 1)                 # A{idx}
                end_a1 = rowcol_to_a1(idx, len(values))        # 例: AA{idx} など
...
async def chinchiro_cmd(interaction: discord.Interaction):
    ...
    async with get_user_lock(interaction.user.id):
        ...
        # ここはログインボーナスのカウントと別なのでそのまま
        # （大吉/大凶カウントは占い用のため、diceでは触らない）
...
BJ_SESSION_TIMEOUT_SEC = 20 * 60  # 20分放置で返金して終了
